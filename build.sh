...
echo
	...
	isoLabel='HXero-'$xerolinuxVersion'-x86_64.iso'

	# setting of the general parameters
	archisoRequiredVersion="archiso 72-1"
	buildFolder=$HOME"/Hxero-build"
	outFolder=$HOME"/HXero-Out"
...
echo
	...
	echo "Removing the old packages.x86_64 file from build folder"
	rm $buildFolder/HXero/packages.x86_64
	rm $buildFolder/HXero/packages-personal-repo.x86_64
	echo

	echo "Copying the new packages.x86_64 file to the build folder"
	cp -f HXero/packages.x86_64 $buildFolder/HXero/packages.x86_64
	echo

	if [ $personalrepo == true ]; then
		echo "Adding packages from your personal repository - packages-personal-repo.x86_64"
		printf "\n" | sudo tee -a $buildFolder/HXero/packages.x86_64
		cat HXero/packages-personal-repo.x86_64 | sudo tee -a $buildFolder/HXero/packages.x86_64
	fi

	if [ $personalrepo == true ]; then
		echo "Adding our own repo to /etc/pacman.conf"
		printf "\n" | sudo tee -a $buildFolder/HXero/pacman.conf
		printf "\n" | sudo tee -a $buildFolder/HXero/airootfs/etc/pacman.conf
		cat personal-repo | sudo tee -a $buildFolder/HXero/pacman.conf
		cat personal-repo | sudo tee -a $buildFolder/HXero/airootfs/etc/pacman.conf
	fi
...
echo
	...
	echo "Changing all references"
	echo
	sed -i 's/'$oldname1'/'$newname1'/g' $buildFolder/HXero/profiledef.sh
	sed -i 's/'$oldname2'/'$newname2'/g' $buildFolder/HXero/profiledef.sh
	sed -i 's/'$oldname3'/'$newname3'/g' $buildFolder/HXero/airootfs/etc/dev-rel
	sed -i 's/'$oldname4'/'$newname4'/g' $buildFolder/HXero/airootfs/etc/hostname
	sed -i 's/'$oldname5'/'$newname5'/g' $buildFolder/HXero/airootfs/etc/sddm.conf

	echo "Adding time to /etc/dev-rel"
	date_build=$(date -d now)
	echo "Iso build on : "$date_build
	sudo sed -i "s/\(^ISO_BUILD=\).*/\1$date_build/" $buildFolder/HXero/airootfs/etc/dev-rel
...
echo
	...
	cd $buildFolder/HXero/
	sudo mkarchiso -v -w $buildFolder -o $outFolder $buildFolder/HXero/
